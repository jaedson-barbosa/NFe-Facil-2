<script lang="ts">
  ...
  let tipo = r[CST]
  if (raiz['ICMSPart']) tipo = 'Part' + tipo
  if (raiz['ICMSST']) tipo = 'ST' + tipo

  $: {
    raiz[el] = {}
    let cst = tipo as string
    let rn = el + cst
    if (cst === '41' || cst === '50') {
      rn = el + '40'
    } else if (cst === 'Part10' || cst === 'Part90') {
      rn = 'ICMSPart'
      cst = cst === 'Part10' ? '10' : '90'
    } else if (cst === 'ST41' || cst === 'ST60') {
      rn = 'ICMSST'
      cst = cst === 'ST41' ? '41' : '60'
    }
...
/>
{#if ['00', '10', '20', '51', '70', '90', 'Part10', 'Part90'].includes(tipo)}
  <Select
    bind:val={r['modBC']}
    opt={tipo == '51'}
  ...
  />
  {#if ['20', '51', '70', '90', 'Part10', 'Part90'].includes(tipo)}
    <InputT
      bind:val={r['pRedBC']}
      opt={tipo != '20'}
...
{#if ['00', '10', '20', '51', '70', '90'].includes(tipo)}
  ...
  />
{/if}
{#if ['10', '30', '70', '90', 'Part10', 'Part90'].includes(tipo)}
  <Select
    bind:val={r['modBCST']}
  ...
  />
{/if}
{#if ['10', '30', '60', '70', '90'].includes(tipo)}
  <InputT
    bind:val={r['vBCFCPST']}
...
{#if ['60', 'ST41', 'ST60'].includes(tipo)}
  ...
  <InputT
    ...
    opt={['ST41', 'ST60'].includes(tipo) ||
      !['vBCSTRet', 'vICMSSubstituto', 'vICMSSTRet'].some((v) => r[v])}
